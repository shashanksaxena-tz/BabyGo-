...
router.get('/:childId', authMiddleware, async (req, res) => {
  try {
    // ⚡ Bolt: Parallelize Child existence check with TimelineEntry query
    // Use .exists() instead of .findOne() to save memory/parsing
    // Use .lean() on the data query to bypass Mongoose hydration
...
router.get('/measurements/:childId', authMiddleware, async (req, res) => {
  try {
    // ⚡ Bolt: Parallelize Child existence check with Measurement query
    // Use .exists() instead of .findOne() to save memory/parsing
    // Use .lean() on the data query to bypass Mongoose hydration
