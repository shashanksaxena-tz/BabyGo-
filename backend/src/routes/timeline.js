...
router.get('/:childId', authMiddleware, async (req, res) => {
  try {
    // ⚡ Bolt Optimization: Parallelized independent database queries and replaced findOne() with exists()
    // Impact: Eliminates sequential blocking (saves ~30-50ms) and avoids hydrating unused child document in memory
...
router.get('/measurements/:childId', authMiddleware, async (req, res) => {
  try {
    // ⚡ Bolt Optimization: Parallelized existence check with data fetch and replaced findOne() with exists()
    // Impact: Reduces endpoint latency by eliminating sequential round-trip to database and saves memory
