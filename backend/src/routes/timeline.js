...
router.get('/:childId', authMiddleware, async (req, res) => {
  try {
    // ⚡ Bolt: Parallelize child existence check and entries fetch, and use .lean() for read-only queries
...
router.get('/measurements/:childId', authMiddleware, async (req, res) => {
  try {
    // ⚡ Bolt: Parallelize child existence check and measurements fetch, and use .lean() for read-only queries
