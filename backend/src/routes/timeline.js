...
router.get('/:childId', authMiddleware, async (req, res) => {
  try {
    const [childExists, entriesData] = await Promise.all([
    ...
    // Map _id to id to support frontend expectations after .lean()
...
router.get('/measurements/:childId', authMiddleware, async (req, res) => {
  try {
    const [childExists, measurementsData] = await Promise.all([
      Child.exists({ _id: req.params.childId }),
