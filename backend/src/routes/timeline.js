...
router.get('/:childId', authMiddleware, async (req, res) => {
  try {
    ...
    // ⚡ Bolt: Parallelize child existence check and timeline entry fetch, use .lean()
    const [childExists, entries] = await Promise.all([
      Child.exists({ _id: childId }),
      TimelineEntry.find({ childId: String(childId) })
        .sort({ date: -1 })
        .limit(100)
        .lean()
    ]);
...
router.get('/measurements/:childId', authMiddleware, async (req, res) => {
  try {
    ...
    // ⚡ Bolt: Parallelize child existence check and measurements fetch, use .lean()
    const [childExists, measurements] = await Promise.all([
      Child.exists({ _id: childId }),
      Measurement.find({ childId: String(childId) })
        .sort({ date: 1 })
        .lean()
    ]);
