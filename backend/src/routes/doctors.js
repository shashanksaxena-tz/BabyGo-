...
router.get('/', authMiddleware, async (req, res) => {
  try {
    ...
    const doctors = await Doctor.find(filter).sort({ rating: -1 }).lean(); // Optimization: Bypasses Mongoose document hydration for faster read performance and lower memory usage.
...
router.get('/recommended/:childId', authMiddleware, async (req, res) => {
  try {
    ...
    if (!child) {
      ...
      const doctors = await Doctor.find({ isActive: true }).sort({ rating: -1 }).lean(); // Optimization: Bypasses Mongoose document hydration for faster read performance and lower memory usage.
      return res.json({
        flaggedDomains: [],
        domainScores: {},
        childName: '',
    ...
    const allDoctors = await Doctor.find({ isActive: true }).sort({ rating: -1 }).lean(); // Optimization: Bypasses Mongoose document hydration for faster read performance and lower memory usage.
