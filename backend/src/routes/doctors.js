...
router.get('/', authMiddleware, async (req, res) => {
  try {
    ...
    // ⚡ Bolt: Using .lean() to bypass document hydration for read-only query
...
router.get('/recommended/:childId', authMiddleware, async (req, res) => {
  try {
    ...
    if (!child) {
      ...
      // ⚡ Bolt: Using .lean() avoids the need to call .toObject()
    ...
    // ⚡ Bolt: Using .lean() avoids the need to call .toObject() on each item
