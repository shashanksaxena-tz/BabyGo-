...
router.get('/', authMiddleware, async (req, res) => {
  try {
    ...
    // ⚡ Bolt: Added .lean() to bypass document hydration for read-only query
    res.json({ doctors });
  } catch (error) {
...
router.get('/recommended/:childId', authMiddleware, async (req, res) => {
  try {
    ...
    if (!child) {
      ...
      // ⚡ Bolt: Added .lean() and removed .toObject() for performance
    ...
    // ⚡ Bolt: Added .lean() for performance since documents are strictly read-only
