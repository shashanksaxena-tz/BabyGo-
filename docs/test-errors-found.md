...
| 7 | CRITICAL | `backend/src/routes/children.js:235` | Timeline entry created without required `date` field. Mongoose validation silently fails, so milestone achievements never create timeline entries. Data loss in production. | **Fixed** |
